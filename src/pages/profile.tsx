...
const Profile: FC = () => {
    ...
    const { values, handleChange, setValues } = user ? useForm({ name: user.name, email: user.email, password: '' }) : useForm({ name: '', email: '', password: '' })
    ...
    const linkActive = ({ isActive }: { isActive: boolean }) => isActive ? `${style.link} ${style.link_active} text text_type_main-medium` : `${style.link}  text text_type_main-medium text_color_inactive`

    const exit = () => {
        dispatch(logout())
    }

    const cancel = (e: SyntheticEvent<Element, Event>) => {
        e.preventDefault()
        if (user) {
            setValues({
                name: user.name,
            ...
            <div className={style.row}>
                <ul className={style.list}>
                    {profileLink ? <li className={style.item}><NavLink to="/profile" className={linkActive}>Профиль</NavLink></li>
                        : <li className={style.item}><NavLink to="/profile" className={`${style.link}  text text_type_main-medium text_color_inactive`}>Профиль</NavLink></li>
                    }
